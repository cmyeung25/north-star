...
- Current repository checkout does **not** contain checked-in onboarding-review or market-entry event exports for this window.
- The fixed-window dashboard/export pipeline was therefore run against the currently available in-environment event set for this checkout, which is empty.
...
| Guardrail | Shown reviews | Fix-success | Review → completed improvement vs previous window | Classification | Reason |
|---|---:|---:|---|---|---|
| `property_usage_missing` | 0 | 0.0% | not measurable | observation only | Weekly review sessions = 0 and shown support = 0. |
| `duplicate_current_home_housing_costs` | 0 | 0.0% | not measurable | observation only | Weekly review sessions = 0 and shown support = 0. |
| `duplicate_rent_expense_inputs` | 0 | 0.0% | not measurable | observation only | Weekly review sessions = 0 and shown support = 0. |
| `mortgage_property_basics_missing` | 0 | 0.0% | not measurable | observation only | Weekly review sessions = 0 and shown support = 0. |
...
- The market-entry board also had no signed-in / signed-out, journey / preset, or experiment-slot cohort traffic in this window, so there is no adjacent cohort signal to support or refute confusion.

### Decision
- **Classification: observation only**
- **Reason:** no weekly support, and the qualitative beta-feedback signal requested by the workflow was not available inside the checked-in analytics evidence for this repository checkout.
...
| Ship new guardrail copy / action-hint tuning from this window alone | observation only | Do not promote this week into product policy. | No usable weekly review evidence exists in this checkout. |
| Treat preset-recovery confusion as a proven product issue | observation only | Keep as beta watch item only. | No cohort evidence and no tagged beta feedback in the reviewed window. |
| Keep the weekly Monday → Monday UTC ritual and export aggregate + locale packs | can action | Continue as operating baseline. | The pipeline generated the correct fixed window and explicit no-data statuses. |
| Require the next review to use exported real event data from the debug board or equivalent internal export | can action | Make real weekly exports a prerequisite for roadmap reprioritization. | Current evidence gap is now explicit and measurable. |
