...
- [x] 下一批模板進入 beta-ready / gated rollout：`mortgage_rate_hike` 與 `move_home` 已補齊 title / description / disabled reason / estimate guide / cost-profile 文案、launcher path、以及 payload 校準，但仍維持 hidden，直至 rollout gate 達標後才開啟 `NEXT_PUBLIC_FF_PLANLAB_DEFERRED_DECISION_TEMPLATES`。
  - [x] `mortgage_rate_hike` 使用者決策問題：如果未來按揭重訂 / 加息，現金流安全邊際會否明顯惡化？beta-ready 定義：由 Plan Lab Add Experiment 進入；只讀 active scenario baseline / existing editable mortgage event；以既有 patch / experiment-group 管線預填較高按揭利率草稿（保守/中位/進取 = +0.75 / +1.5 / +2.5 厘）；沿用既有 KPI delta summary，不改 engine interface。
  - [x] `move_home` 使用者決策問題：如果把搬屋 / 換樓時點延後或重排，短中期現金流壓力會否改善？beta-ready 定義：由 Plan Lab Add Experiment 進入；只讀 active scenario baseline / existing editable housing event；以既有 patch / experiment-group 管線預填較後住屋時點草稿（保守/中位/進取 = +6 / +12 / +18 個月）；沿用既有 KPI delta summary，不改 engine interface。
  - [ ] Rollout gate：只有當 onboarding start rate ≥ 85%、onboarding review → completed conversion 在兩個連續完整週窗都有足夠 support（review sessions ≥ 20，且未觸發 locale skew / support warning）並被 weekly board 判定為 stable，才可在環境層打開 `NEXT_PUBLIC_FF_PLANLAB_DEFERRED_DECISION_TEMPLATES`；未達標一律維持 hidden。
  - [ ] 風險：若 weekly board evidence 仍不足，切勿因 copy 已完成就提早 expose；兩個模板都必須 fail closed 到既有 disabled reason，不得 invent mortgage / housing baseline data。
...
- [ ] 下一步：先按 PR1 固定好的 weekly artifacts 持續補齊真實 export，再依 beta feedback 與固定 weekly review workflow 結果調整 guardrail 文案、誤報率與 severity/copy 策略。現時 current / previous 兩個完整週窗都無 review sessions，所以四條 priority rules 的 show-rate、fix-success、review→completed trend 仍只屬 observation-only heuristic；roadmap 排序暫不因這些數字改動。
4. Presets 接入主流程
...
- [ ] 下一步（排序更新）：preset recovery confusion 仍未有真實 cohort / tagged beta feedback 證據；因此 roadmap 只保留「先驗證、後決策」狀態說明，不在本輪提升為新功能承諾或額外 analytics scope。
