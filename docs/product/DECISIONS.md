...
- Decision: Treat app preset suggestions as setup/recovery guidance, not template-picking. Reuse a shared summary presenter/view-model so member create and onboarding start/resume both render the same four-step summary structure (`audience` → `starting context` → `ETA` → `outcome`). On app recovery surfaces, only show replace warning / replace CTA when an onboarding draft already exists, and always restate that the action only replaces the active scenario’s onboarding draft starting point, never writes baseline data directly, and never marks onboarding complete.
