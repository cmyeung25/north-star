...
describe("milestone migration", () => {
  it("maps legacy members[].milestones into scenario.milestoneEvents when milestoneEvents is empty", () => {
    const normalized = normalizeScenario(
      buildScenario({
        members: [
          {
    ...
    expect(normalized.milestoneEvents).toHaveLength(1);
    expect(normalized.milestoneEvents?.[0]).toMatchObject({
      legacy: true,
      ...
      metadata: {
        ...
        sourceEventId: "legacy-source-1",
        memberId: "member-1",
        memberMilestoneId: "legacy-ms-1",
      },
    });
  });

  it("keeps scenario.milestoneEvents as source of truth when non-empty", () => {
    const normalized = normalizeScenario(
      buildScenario({
        members: [
          {
    ...
    expect(normalized.milestoneEvents).toHaveLength(1);
    expect(normalized.milestoneEvents?.[0]?.id).toBe("evt-ms-1");
    expect(normalized.milestoneEvents?.[0]?.notes).toBe("Canonical");
  });
});
