...
import { validateAndNormalizeScenarioDraft } from "./validate";
import type { ScenarioCreatePayload, ScenarioDraft } from "./types";
...
export const compileScenarioCreatePayload = (
  ...
  const { normalizedDraft, issues } = validateAndNormalizeScenarioDraft(draft);
  ...
  return {
    ...
    events: Array.isArray(rawEventsPayload.events) ? (rawEventsPayload.events as ScenarioEvent[]) : [],
    meta: {
      ...(context.metaBase ?? {}),
