...
Onboarding completeness score v1 – The onboarding review/completeness layer now evaluates five input groups (`household`, `income`, `fixedExpenses`, `housing`, `assetsLiabilities`) and outputs a UI summary model with overall levels `ready` / `needs_attention` / `incomplete`. It must remain engine-independent, read only the active onboarding draft + active scenario context, and treat auto-suggested salary rows as `needs_attention` rather than fully complete.

Onboarding guardrails v1 – The onboarding rules layer now separately emits a guardrail summary model for UI consumption. Each rule must define `id`, `severity`, `messageKey`, `actionHintKey`, and `target step/section`, and must remain engine-independent plus scenario-scoped. v1 is intentionally focused on housing/property errors (`key_missing`, `obvious_conflict`, `basic_inconsistency`, `potential_double_counting`) such as missing property usage, incomplete mortgage core fields, self-use/rental conflicts, housing-state cost mismatch, and duplicate housing spend.

Onboarding review / submit UX + analytics v1（2026-03-20）– onboarding v3 review step must present completeness + guardrails before submit, provide per-group / per-guardrail return-to-step actions, and keep submit/save feedback explicit (`validating` / `saving` / `redirecting`) so the user never wonders whether the baseline was saved. Review IA must show an overall guardrail summary plus three severity groups (`critical / must fix`, `warning / review recommended`, `info / heads-up`) derived only from `guardrailSummary.items[].severity`; do not re-implement business severity rules in the component. Funnel analytics for this step must stay metadata-only: allowed fields are locale, review/completeness/guardrail levels, counts, guardrail id/category/severity, and target step/section; never send money amounts, asset values, or other financial payload details.

Onboarding guardrail analytics review pack v1（2026-03-20）– onboarding funnel contract now allows two additional safe review-only metadata fields: `reviewSessionId` (ephemeral review-pass correlation key) and `reviewSourceContext` (`initial_review` / `returned_from_fix`). `guardrail_fixed` only counts when the user clicked a fix CTA, returned to a later review pass, and that rule no longer appears. Keep payloads allowlisted + metadata-only; do not add scenario ids, financial values, persistent fix progress, or analytics-driven state machines.

Onboarding guardrail weekly calibration v1.2（2026-03-20）– when review-pack output suggests a rule is high-show and/or low-fix-success, prefer severity/copy/target-step tuning before adding more rules. `duplicate_current_home_housing_costs` is now a non-blocking `info` reminder (not `warning`) because it usually signals current-home path ambiguity rather than a guaranteed error. Current calibration priority sets are: top-shown candidates (`property_usage_missing`, `duplicate_current_home_housing_costs`, `duplicate_rent_expense_inputs`), low-fix-success candidates (`duplicate_current_home_housing_costs`, `duplicate_rent_expense_inputs`, `mortgage_property_basics_missing`), and review-without-completion watchlist (`duplicate_current_home_housing_costs`, `mortgage_core_fields_missing`, `property_usage_missing`). Keep rules engine-independent, scenario-scoped, and analytics metadata-only.

Onboarding guardrail weekly review workflow v1（2026-03-21）– the operating ritual now defaults to the previous full Monday→Monday UTC week and must always review aggregate + locale packs first, then the four fixed focus rules (`property_usage_missing`, `duplicate_current_home_housing_costs`, `duplicate_rent_expense_inputs`, `mortgage_property_basics_missing`). Confidence checks are mandatory before actioning a rule: under 20 review sessions in the week => directional only; under 5 shown review sessions for a rule => observation only; locale share above 70% => annotate locale skew. Persona / preset / journey bias must be cross-checked via the separate market-entry weekly board because onboarding analytics intentionally do not carry those fields.

Focused UX calibration step 2（2026-03-21）– once two full weekly review windows are available, the next pass must stay focused on the fixed four onboarding guardrails (`property_usage_missing`, `duplicate_current_home_housing_costs`, `duplicate_rent_expense_inputs`, `mortgage_property_basics_missing`) and tune copy / action hint / target-step clarity before reconsidering severity. In parallel, the three app preset recovery surfaces (onboarding start/resume, dashboard recovery, settings data-management reset) must explicitly state that they are not Plan Lab experiments or Money event create/edit flows; dashboard may use stronger recovery wording, while settings must stay on the shorter reset/restart tone.
...
Overview → Plan Lab first-decision path v1（2026-03-21）– once an active scenario is baseline-ready and no onboarding recovery banner is showing, Overview may surface a dedicated “first major decision comparison” CTA. This CTA may only open Plan Lab decision-template mode via allowlisted route intent (`openDecisionTemplates=1` plus optional allowlisted `decisionTemplate` id for recommendation highlighting). It must not auto-apply a template, write baseline data, or introduce cross-scenario recommendation state; recommendation heuristics must stay lightweight and active-scenario-scoped (for example persona focus + current housing baseline).

Plan Lab deferred decision templates v1 contract（2026-03-20）– the next two roadmap templates are now defined as beta-gated contracts only: `mortgage_rate_hike` and `move_home`. They must stay hidden from the user-facing picker until onboarding `start` and `review → completion` beta metrics are stable. Once enabled, both still enter from Plan Lab Add Experiment, read only the active scenario baseline plus existing editable housing/mortgage events, write through the existing Plan Lab patch / experiment-group path, and reuse the existing KPI delta summary. Availability must fail closed: no editable mortgage event => disable `mortgage_rate_hike`; no editable housing event => disable `move_home`; never invent cross-scenario data or add engine interfaces.
...
App preset extension IA v1 beta contract（2026-03-21）– app 內若要承接同一種 preset / onboarding-prefill 心智模型，只可放在 active-scenario 的 setup / recovery surfaces：`scenario onboarding start / resume`、`overview/dashboard onboarding-incomplete recovery banner`、`scenario settings → data management reset`。三者的共同結論都必須是「為 active scenario 建立或取代 scenario-scoped onboarding draft 起點，然後回到 onboarding 完成確認」；不得直接寫 baseline、不得把 preset 解讀為已完成 scenario、不得改變登入後先到 `/{locale}/member/cases` 的規則。避免與既有 creation mental models 混淆：preset suggestions 不得放進 Plan Lab Add Experiment、Money add-event / template picker、或其他 baseline 事件 create/edit drawers；onboarding/dashboard 可用 journey guidance + ETA + outcome copy，settings reset 則只用較克制的 replace/restart copy 與覆蓋警告。

Onboarding preset suggestion beta entry 1（2026-03-21）– `/app/case/[caseId]/scenario/[scenarioId]/onboarding` start / resume shell 現已上線最小版 preset suggestion 卡片，只在 active scenario `meta.onboarded` 未完成時顯示。CTA 必須走既有 onboarding draft write path：建立或取代 scenario-scoped onboarding draft 起點後，停留在同一 onboarding wizard 繼續完成；不得寫 baseline event、不得標記 onboarding complete。Guardrail 測試需覆蓋「只出現在 onboarding-incomplete surface」與「Plan Lab / Money / baseline event drawers 不得重用此 helper / component」。

Onboarding preset suggestion beta entry 1.1 copy rule（2026-03-21）– onboarding start / resume preset suggestions 現需與 member create dialog 對齊同一個 summary mental model：先講 `audience / starting context`，再講 `ETA`，最後講 `outcome after onboarding`；語氣必須是 setup / recovery，不可像 Plan Lab experiment template 或 Money event template。若 active scenario 已存在 onboarding draft，才顯示 replace warning / replace CTA，並再次明示「只取代 onboarding draft 起點、不直接寫 baseline、也不代表 onboarding 已完成」。這套 copy / IA 應作為之後 dashboard recovery banner 的預設規則。

Onboarding preset suggestion beta entry 2（2026-03-21）– overview/dashboard recovery banner 現已上線第二個 app 內 preset surface，但只限「已可進入 dashboard 的 active scenario，且仍有 onboarding recovery gaps」時顯示。Banner + preset cards 必須重用入口 1 的 allowlist、journey guidance summary 與 onboarding draft write path；CTA 只會建立 / 取代 active scenario 的 scenario-scoped onboarding draft 起點，然後立即返回 onboarding 繼續完成確認，不得直接寫 baseline、不得直接完成 scenario，也不得新增 dashboard-specific preset / analytics 規則。Guardrail 測試需同時覆蓋 onboarding helper 與 dashboard recovery helper 均不得出現在 Plan Lab Add Experiment、Money add-event / template picker、或其他 baseline event drawers。

Scenario settings preset recovery beta entry 3（2026-03-21）– `scenario settings → Data Management` 現已上線第三個 app 內 preset surface，但只限 Data Management / reset 類操作區塊內顯示。此入口必須重用既有 six-seed allowlist、共享 journey summary presenter 與 onboarding draft write path；CTA 只會建立 / 取代 active scenario 的 scenario-scoped onboarding draft 起點，然後導回既有 onboarding flow，不得直接寫 baseline、不得標記 onboarding complete。文案要比 onboarding / dashboard 更克制：無既有 draft 時只用 start-from-preset 語意；有既有 draft 時才顯示 replace warning / replace CTA 與覆蓋警告。

Source-guard coverage v3（2026-03-21）– onboarding helper、dashboard recovery banner、settings data-management recovery helper 三者都必須維持 source-guard 測試，確認不會出現在 Plan Lab Add Experiment、Money add-event / template picker、或任何 baseline event create/edit drawer（包括 cashflow / housing / insurance / loan / asset / liability 等編輯抽屜）。


Market-entry journey contract v1.2 - Public persona/sample-journey CTAs must use `/{locale}/member/cases?journey={journeyId}&preset={presetId}` as the only supported handoff contract. `journey` expresses entry narrative only; `preset` must stay within the member preset allowlist and may only initialize the member create dialog. The allowlisted journey ids, each journey’s primary preset mapping, and the explicit blank-fallback rule must live in one canonical source shared by marketing CTA builders and member query parsing. Invalid or unsupported mappings must fall back to blank create flow rather than exposing hidden seeds, bypassing `/member/cases`, or auto-marking onboarding complete. Signed-out users may authenticate first, but after auth they must still return to `/{locale}/member/cases` and rehydrate the same sanitized entry intent there.

Market-entry measurement layer v1（2026-03-20）- Publishability review for public sample journeys now depends on a metadata-only, vendor-agnostic funnel contract. Allowed payload fields are only `locale`, `journeyId`, `presetId`, and `isSignedIn`; never add case ids, scenario ids, or financial values. Required journey events now include `market_landing_view`, `sample_journey_impression`, `journey_cta_click`, `case_created`, `preset_create_started`, `preset_create_submitted`, and `onboarding_started`. `sample_journey_impression` must fire once per card exposure path per mount (not every re-render), and `case_created` must only fire after create-case truly succeeds while still preserving the `/member/cases` → onboarding handoff without direct scenario/onboarding shortcuts.

Market-entry weekly dashboard / export productization v1（2026-03-21）– the fixed weekly review ritual is now backed by product code: `marketEntry.ts` persists metadata-only events locally for internal review, propagates experiment-slot attribution across the marketing → member → onboarding handoff, and now supports a metadata-only `onboarding_completed` event so signed-in / signed-out completion deltas can be reviewed on the market-entry board. Internal debug surface `/{locale}/debug/weekly-review` renders the fixed onboarding + market-entry weekly dashboard, while `weeklyProductAnalyticsDashboard.ts` exports stable JSON / CSV outputs for PM / UX / Product without introducing new case / scenario persistence. Keep all payloads metadata-only and do not add financial fields, case ids, or scenario ids.

Market-entry experiment slot governance v1（2026-03-21）- copy testing for public entry must stay inside a vendor-agnostic content-assembly layer. Stable slot keys are `landing.hero.value_prop`, `landing.persona.cta_summary`, and `landing.sample_journey.summary`; variant names must use lowercase snake_case with `_v{n}` suffix (e.g. `control_v1`, `clarity_first_v1`, `decision_first_v1`). Slots may change copy keys and block ordering only. They must not change the `journey + preset` handoff contract, signed-in/out auth return behavior, preset allowlists, invalid mapping → blank flow fallback, or expose hidden presets. If analytics needs experiment metadata, only allowlist `experimentSlotKey` and `experimentVariant`, and keep them metadata-only.
...
- Rental life bundle parity v1 (2026-03-09): `life_rental_plan` must emit one rent housing event plus setup one-off cashflows (deposit / agent fee) under the same `source.bundleInstanceId` / `source.templateId`. Plan Lab and Money should treat rental and home purchase consistently as life-bundle cards/summaries; if older data lacks `bundleInstances[].wizardInput`, fallback wizard hydration may only read active-scenario bundle events.
