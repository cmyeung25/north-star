...
- Date: 2026-03-06
  - Context / Why: `members[].milestones` still affected runtime highlights and timeline write paths, while Overview had already shifted to `scenario.milestoneEvents`; product required one-way migration and Overview+Breakdown single-source milestone markers.
  - What changed: Added legacy migration helper `migrateLegacyMemberMilestonesToEvents` (with deterministic legacy IDs) and wired it into `normalizeScenario` to auto-convert member milestones into marker-mode `milestoneEvents`; introduced `MilestoneEvent.mode` (`impact|marker`) + optional `memberId` and marker-safe compile behavior; removed monthly highlight dependence on legacy milestone computation and switched to selector-fed `milestoneMarkers`; updated Timeline retirement milestone sync to write marker events via `applyMilestoneEvent/removeMilestoneEvent`; added Overview milestone-management drawer (create/edit/delete marker events) and passed unified `milestoneMarkers` into Monthly Breakdown modal for synchronized highlights; updated members legacy notice copy to reflect auto-migration.
  - Affected paths: apps/web/src/domain/milestoneEvents/types.ts; apps/web/src/domain/milestoneEvents/compiler.ts; apps/web/src/domain/milestoneEvents/migrateLegacyMemberMilestones.ts; apps/web/src/domain/milestoneEvents/__tests__/migrateLegacyMemberMilestones.test.ts; apps/web/src/store/scenarioStore.ts; apps/web/src/domain/timeline/getMonthlyHighlights.ts; apps/web/src/domain/timeline/buildOverviewTimelineMarkers.ts; apps/web/src/domain/timeline/__tests__/buildOverviewTimelineMarkers.test.ts; apps/web/src/domain/timeline/__tests__/getMonthlyHighlights.test.ts; apps/web/components/timeline/TimelineEventDrawer.tsx; apps/web/app/[locale]/overview/OverviewClient.tsx; apps/web/components/MonthlyBreakdownModalHost.tsx; apps/web/components/ProjectionDetailsModal.tsx; apps/web/messages/zh-HK.json; apps/web/messages/en.json; AGENTS.md
  - Guardrails for next agent: Treat `memberMilestonesMigratedAt` as cutover flag and avoid re-reading legacy member milestones once set; keep Overview/Breakdown milestone rendering on `buildOverviewTimelineMarkers` + `milestoneMarkers` only; marker-mode events must never emit compiler ops or fake financial payload.
  - Validation commands run: pnpm -w lint; pnpm -w typecheck; pnpm -w test; pnpm -w --filter web build

- Date: 2026-03-06
  - Context / Why: Milestone marker features existed but maintenance UX remained single-action and fragmented across Overview/Timeline entry points.
  - What changed: Upgraded Overview right drawer into a milestone management center with single-source list rendering from `scenario.milestoneEvents` (marker mode), search/filter (member/template/source/status/month range), upcoming-first sorting, inline create/edit form, template shortcuts, duplicate guard (`memberId + month + template`), system-derived milestone governance (lock core edits, source-jump CTA, delete re-generation warning), toast feedback, delete undo window, and unsaved-close confirmation. Added deep-link query entry (`?milestones=manage`) and connected Timeline overview tab CTA to open this manager. Updated Timeline overview milestone table/cards to read marker milestones directly from scenario store for consistency with Overview markers.
  - Affected paths: apps/web/app/[locale]/overview/OverviewClient.tsx; apps/web/components/timeline/TimelineDesktop.tsx; apps/web/components/timeline/TimelineMobile.tsx; apps/web/messages/zh-HK.json; apps/web/messages/en.json; AGENTS.md
  - Guardrails for next agent: Keep milestone management scoped to `scenario.milestoneEvents` marker entries (do not reintroduce derived dual-source rows in Overview/Timeline); keep system-derived milestones non-editable in manager and route edits to source Timeline events; preserve manager deep-link contract `?milestones=manage` for cross-page entry consistency.
  - Validation commands run: pnpm -w lint; pnpm -w typecheck; pnpm -w test; pnpm -w --filter web build

- Date: 2026-03-07
  - Context / Why: Milestone management entry lived in Overview drawer while Settings IA now requires dedicated management tabs with clearer separation between members and milestones.
  - What changed: Added Settings tab key `milestones` and mounted new `MilestoneManagementPanel` in Scenario Settings; renamed settings tabs to scenario assumptions / member management / milestone management / data management via i18n keys; redirected Overview + Timeline milestone manager CTAs to `#milestones` in Scenario Settings; removed members-tab event source/link-state tags (baseline-only/orphaned) from card metadata; added missing members i18n key `memberEventsPeriod` (zh-HK/en).
  - Affected paths: apps/web/components/settings/MilestoneManagementPanel.tsx; apps/web/components/settings/ScenarioSettingsWorkspace.tsx; apps/web/components/people/PeopleWorkspace.tsx; apps/web/app/[locale]/overview/OverviewClient.tsx; apps/web/components/timeline/TimelineDesktop.tsx; apps/web/components/timeline/TimelineMobile.tsx; apps/web/components/settings/__tests__/ScenarioSettingsWorkspace.members.test.tsx; apps/web/messages/zh-HK.json; apps/web/messages/en.json; AGENTS.md
  - Guardrails for next agent: Keep milestone maintenance UX under Settings `#milestones` as primary entry; keep members tab focused on member profile + assignment visibility only (no internal source/link-state jargon badges); when adding settings tabs, update `SettingsTabKey`, PeopleWorkspace tab mapping, and locale keys together.
  - Validation commands run: pnpm -w lint; pnpm -w typecheck; pnpm -w test; pnpm -w --filter web build
